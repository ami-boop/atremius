...
- AI/прогноз производительности считается локально в браузере (на основе истории метрик + погоды Open-Meteo + body insights).
...
- `src/api/weather.js`:
  ...
  - тянет 8 дней из Open-Meteo, использует дни 1..7 (без текущего дня).
