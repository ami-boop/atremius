...
const generateSeries = (max, productiveLabel = "ВЫСОКАЯ ПРОДУКТИВНОСТЬ") => [
...
const day = {
  ...
  physiology: {
    ...
    neck: { status: null, comment: "" },
    shoulder_l: { status: null, comment: "" },
  ...
  momentumChart: {
    focus: [
      ...
      { label: "ВТ", value: 70, mode: "ВЫСОКАЯ ПРОДУКТИВНОСТЬ" },
      { label: "СР", value: 60, mode: "СТАБИЛЬНО" },
      { label: "ЧТ", value: 85, mode: "ВЫСОКАЯ ПРОДУКТИВНОСТЬ" },
      { label: "СГД", value: 78, mode: "ВЫСОКАЯ ПРОДУКТИВНОСТЬ" },
      { label: "СБ", value: 0, mode: null },
      { label: "ВС", value: 0, mode: null },
    ],
    stress: [
      ...
      { label: "ЧТ", value: 30, mode: "ВЫСОКАЯ ПРОДУКТИВНОСТЬ" },
      { label: "СГД", value: 35, mode: "ВЫСОКАЯ ПРОДУКТИВНОСТЬ" },
      { label: "СБ", value: 0, mode: null },
      { label: "ВС", value: 0, mode: null },
    ],
  },
  momentumChartAnalytics: {
    focus: generateSeries(100),
    stress: generateSeries(100),
    social: generateSeries(10),
    activity: generateSeries(300),
    sleep: generateSeries(10),
    concentration: generateSeries(10),
  },
  forecast: {
    days: [
...
async function main() {
  ...
  await setDoc(doc(db, "profile", "app"), profile);
  console.log("Wrote profile/app");

  await setDoc(doc(db, "days", "2026-04-19"), day);
  console.log("Wrote days/2026-04-19");
